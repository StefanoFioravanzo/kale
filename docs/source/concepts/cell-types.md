...
| Tag                         | Example                         | Effect                                                                 |
| --------------------------- | ------------------------------- | ---------------------------------------------------------------------- |
| `imports`                   | `imports`                       | Cell is prepended to every pipeline step. **All `import` statements must live in an `imports` cell.** |
| `functions`                 | `functions`                     | Cell is prepended to every step after `imports`. Put function and class definitions here. |
| `pipeline-parameters`       | `pipeline-parameters`           | Variables defined here become KFP pipeline parameters.                 |
| `pipeline-metrics`          | `pipeline-metrics`              | `print()` statements in the cell are converted to KFP pipeline metrics. |
...
| `skip`                      | `skip`                          | Cell is excluded from the pipeline entirely.                           |
...
```{note}
`block:<name>` is a deprecated alias for `step:<name>`. It still works, but
new notebooks should use `step:`.
```
