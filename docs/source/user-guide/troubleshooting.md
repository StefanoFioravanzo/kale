...
Pipeline steps need access to a shared volume to pass data around. If your
storage class doesn't support `ReadWriteMany`, multiple steps will
contend for the PVC and KFP will fail to schedule them.

**Fix.**

- Install a storage class that supports `RWX` (NFS, EFS, Azure Files,
  Longhorn, ...), or
- Configure the pipeline so that at most one step runs at a time (serial
  dependencies only — no fan-out), or
- Open the generated `.kale/<name>.kale.py`, find the volume definition,
  and change the access mode from `ReadWriteMany` to `ReadWriteOnce`.
