...
The **Compile and Run** button invokes the Kale backend via JSON-RPC (the
`kale_server` process that JupyterLab spawns). The backend compiles the
pipeline with exactly the same code path as `kale --nb ...` on the CLI, then
uploads and runs it against the configured KFP host.
...
See the [](../examples.md) gallery for notebooks that follow this pattern.
